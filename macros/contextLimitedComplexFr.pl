...
sub _contextLimitedComplexFr_init {LimitedComplex::Init()}; # don't load it again
...
sub _check {
  ...
  if ($self->{lop}->isRealNumber && $self->{rop}->isRealNumber) {
    ...
    Value::Error("La constante 'i' peut apparaître qu'une seule fois dans votre expression")
  ...
  $self->Error("Votre réponse devrait être sous la forme %s",$self->theForm)
}
...
sub _check {
  ...
  $self->Error("Impossible de prendre la valeur absolue de nombres complexes dans ce contexte");
}
