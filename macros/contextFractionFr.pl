...
sub _contextFractionFr_init {context::Fraction::Init()};
...
sub Init {
  ...
  $context->{error}{msg}{"Vous n'êtes pas autorisés à écrire des nombres décimaux dans ce problème"} =
...
sub _check {
  ...
  $self->Error("Le numérateur d'une fraction doit être un entier")
    unless $self->{lop}->class =~ /INTEGER|MINUS/;
  $self->Error("Le dénominateur d'une fraction doit être un entier (non négatif)")
    unless $self->{rop}->class eq 'INTEGER';
  $self->Error("Le numérateur doit être plus petit que le dénominateur dans une fraction propre")
...
sub reduce {
  ...
  $self->Error("Division par zéro"), return $self if $self->{rop}{isZero};
...
sub _check {
  ...
  if ($isFraction) {
    $self->Error("Les nombres mixtes ne sont pas autorisés: vous devez écrire votre fraction sous forme de rapport de deux entiers")
...
sub div {
  my ($self,$l,$r,$other) = Value::checkOpOrderWithPromote(@_);
  my (($a,$b),($c,$d)) = ($l->value,$r->value);
  Value::Error("Division par zéro") if $c == 0;
...
sub cmp_class {"une fraction de deux entiers"}
...
sub cmp_postprocess {
  ...
  return if $ans->{isPreview} ||
	    ...
	    $student->isReduced;
  $ans->score(0);
  $self->cmp_Error($ans,"Votre fraction n'est pas réduite") if $ans->{showFractionReduceWarnings};
}
